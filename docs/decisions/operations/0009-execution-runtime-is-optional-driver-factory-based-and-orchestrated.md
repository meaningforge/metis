...
`PhysicalQuery` handoff remain durable context.
...
canonical `PhysicalQuery`. It does not open database connections or depend on
live result rows.

Execution Runtime is optional. It consumes already-compiled `PhysicalQuery`
...
Executor
    engine-neutral contract that executes canonical PhysicalQuery artifacts
```
...
- Semantic compilation and runtime execution share the same `PhysicalQuery`
