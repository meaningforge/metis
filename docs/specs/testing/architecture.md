...
15. **Package dependency guards enforce ownership.** `renderer/sql` imports no Metis package; `compiler/artifact` imports only its closed value dependencies; `renderer`, `planner/conversion`, and `execution/driver` do not import root `compiler`. Root `compiler` alone owns generic compilation orchestration, and `compiler/pipeline` must not reappear.
