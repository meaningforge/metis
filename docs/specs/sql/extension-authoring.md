...
| --- | --- |
| `renderer.Renderer` | `renderer.Capabilities`, `sql.SQLDialect`, `sql.SQLQuery`, `sql.QueryParameter`, and `sqlplan.Plan` |
| `driver.Executor` | `artifact.CompiledQuery`, `artifact.OutputSchema`, `artifact.OutputColumn`, `artifact.OutputColumnKind`, `sql.SQLQuery`, `sql.SQLDialect`, and `sql.QueryParameter` |
...
func (Renderer) Render(*sqlplan.Plan) (sql.SQLQuery, error)
```
