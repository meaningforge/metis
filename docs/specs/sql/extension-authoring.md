...
| --- | --- |
| `renderer.Renderer` | `renderer.Capabilities`, `sql.SQLDialect`, `sql.SqlStatement`, `sql.QueryParameter`, and `sqlplan.Plan` |
| `driver.Executor` | `artifact.CompiledQuery`, `artifact.OutputSchema`, `artifact.OutputColumn`, `artifact.OutputColumnKind`, `sql.SqlStatement`, `sql.SQLDialect`, and `sql.QueryParameter` |
...
func (Renderer) Render(*sqlplan.Plan) (sql.SqlStatement, error)
```
