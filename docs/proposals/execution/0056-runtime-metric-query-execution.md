...
`CompiledQuery` is not a replacement for `SqlStatement` as the physical query
...
8. `CompiledQuery.Query` remains the canonical `SqlStatement`; `CompiledQuery.OutputSchema` travels with it across the execution boundary and governs result normalization.
...
The compiler packages the resulting `SqlStatement` together with its
...
- application orchestration does not reconstruct or independently mutate `SqlStatement` or `OutputSchema`.
...
### 23.5 Pass `SqlStatement` and `OutputSchema` separately to Runtime
...
5. `CompiledQuery { SqlStatement + OutputSchema }` as the atomic compiler/execution handoff;
