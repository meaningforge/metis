...
`SqlStatement`, and returns normalized query results. `attribute_metric` owns a
...
Executor
    Metis' small engine-neutral SqlStatement execution contract
```
...
ownership, the `SqlStatement` handoff, and the task-oriented public capability
direction.
...
Stopping permanently at `SqlStatement` pushes orchestration and analytical
...
Domain Planner
       |
       v
Analytical query / SqlStatement bundle
...
Orchestration Layer
          ...
          +-- dispatch SqlStatement to Execution Runtime
...
artifact contract. Execution Runtime consumes `SqlStatement`; it does not
...
invokes Semantic Core, receives one canonical `SqlStatement`, resolves the
...
## 8. SqlStatement is the execution boundary
...
AttributionBundle
      |
      +-- SqlStatement(total/evidence as required)
      +-- SqlStatement(region)
      +-- SqlStatement(channel)
      +-- SqlStatement(product)
```
...
4. **Executor** is the small engine-neutral runtime contract returned by a
   DriverFactory and used to execute canonical `SqlStatement` artifacts.
...
checks. Resolved values MUST NOT enter semantic requests, plans, `SqlStatement`,
...
`compile_sql` before executing the resulting `SqlStatement`.
...
2. Runtime execution consumes canonical `SqlStatement` output rather than
