...
SqlStatement           = rendered execution artifact
```
...
| `SqlStatement` | request scoped compiler artifact | rendered physical query plus canonical output schema | semantic discovery, runtime execution state |
...
type Renderer interface {
    ...
    Render(ctx context.Context, plan *SQLPlan) (compiler.SqlStatement, error)
}
```
...
```text
renderer.Render(SQLPlan) -> SqlStatement
```
