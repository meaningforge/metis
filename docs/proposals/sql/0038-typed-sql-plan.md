...
```text
SemanticEvaluation -> SemanticPlan -> SQLPlan -> SqlStatement
```
...
- a change to `compiler.SqlStatement` or `compiler.OutputSchema` contracts.
...
compiler/      final SqlStatement and OutputSchema contracts
```
...
including parameter values that change the returned `compiler.SqlStatement`. It is
...
type Dialect interface {
    Name() string
    Render(*sqlplan.Plan) (compiler.SqlStatement, error)
}
```
...
chooses placeholder syntax and returns `compiler.SqlStatement.Parameters` in exact
...
18. `compiler.SqlStatement`, `compiler.SqlStatement`, `compiler.OutputSchema`,
