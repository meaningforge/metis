...
`sql.SqlStatement` is not another plan IR; it is the already-rendered SQL text,
...
`RendererCapabilities`, `QueryParameter`, `SqlStatement`, `OutputSchema`, and
...
renderer/
    ...
    sql/                       shared rendering and SqlStatement values
...
| --- | --- | --- |
| `renderer/sql` | shared SQLPlan rendering and query values: `SQLDialect`, parameters, `SqlStatement` | semantic meaning, SQLPlan nodes, Renderer registry, compilation orchestration, execution |
...
| `renderer` | SQL Renderer contract, Renderer capabilities, registry, and dialect-specific `sqlplan.Plan -> sql.SqlStatement` rendering | semantic resolution, planner orchestration, final `CompiledQuery`, DataSource routing, execution |
...
The package name does not mean that `sql.SqlStatement` is a physical plan.
...
and its output is `sql.SqlStatement`.
...
Renderer contract itself guarantees a `sql.SqlStatement` result, an
...
Returning one `sql.SqlStatement` keeps SQL text, SQL dialect, and parameters as
...
- root `compiler` only receives the already-derived schema and composes it with
  the rendered `sql.SqlStatement`; it must not re-derive, reorder, reinterpret,
  or infer schema from rendered SQL.

Moving these types must not weaken the existing atomic
`CompiledQuery { SqlStatement + OutputSchema }` contract.
...
Driver SPI
    ...
    sql.SqlStatement and reachable parameter/dialect types
```
...
sql.SqlStatement          rendered SQL text + SQLDialect + parameters
artifact.CompiledQuery     rendered query + semantic OutputSchema
compiler                   orchestration that constructs CompiledQuery
renderer                   dialect-specific SQLPlan -> SqlStatement conversion
...
returns `sql.SqlStatement`, `Outputs: [SQL]` is a tautology rather than a real
capability boundary.
...
`sql.SqlStatement` and `CompiledQuery` are consumed outside rendering.
...
1. Add `renderer/sql` with `SQLDialect`, `QueryParameter`, and `SqlStatement`.
...
3. Change Renderer signatures and implementations to use `sql.SQLDialect`,
   `sql.QueryParameter`, and `sql.SqlStatement`.
4. Return one owned `sql.SqlStatement` from `Renderer.Render`.
...
- current docs explicitly distinguish `SQLPlan`, `sql.SqlStatement`,
