...
func TestUnusedJoinEliminationPrunesTopLevelJoinsForStagedOnlyRequirements(t *testing.T) {
	...
	input := &semanticplan.SemanticPlan{
		...
		Requested: []string{"hidden_metric"},
		Nodes: []semanticplan.SemanticPlanNode{semanticplan.SourceAggregateNode{
			Base:   semanticplan.SemanticPlanNodeBase{ID: "hidden_metric"},
			Source: semanticplan.SemanticSourceState{RequiredDatasets: []string{"orders", "customer"}},
		}},
	}
