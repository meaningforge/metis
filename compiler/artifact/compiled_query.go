...
type CompiledQuery struct {
	SqlStatement sql.SqlStatement `json:"sql_statement"`
...
// storage.
func NewCompiledQuery(query sql.SqlStatement, schema OutputSchema) (*CompiledQuery, error) {
	return SnapshotCompiledQuery(&CompiledQuery{SqlStatement: query, OutputSchema: schema})
}
...
func SnapshotCompiledQuery(compiled *CompiledQuery) (*CompiledQuery, error) {
	...
	if compiled.SqlStatement.Dialect == "" {
		return nil, fmt.Errorf("physical SQL query dialect is required")
	}
	if strings.TrimSpace(compiled.SqlStatement.SQL) == "" {
		return nil, fmt.Errorf("physical SQL query text is required")
	}
	query, err := snapshotSqlStatement(compiled.SqlStatement)
	if err != nil {
		return nil, err
	}
...
func snapshotSqlStatement(query sql.SqlStatement) (sql.SqlStatement, error) {
	snapshot := query
	...
	for index, parameter := range query.Parameters {
		...
		if err != nil {
			return sql.SqlStatement{}, fmt.Errorf("parameter %d: %w", index, err)
		}
