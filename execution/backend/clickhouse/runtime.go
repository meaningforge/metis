...
func newDataSourceRuntime(db *sql.DB) *dataSourceRuntime { return &dataSourceRuntime{db: db} }

func (r *dataSourceRuntime) Acquire(ctx context.Context) (driver.Executor, error) {
	if r == nil || r.db == nil {
		return nil, fmt.Errorf("ClickHouse DataSource runtime is not initialized")
	}
	if ctx != nil {
	...
	r.mu.RUnlock()
	if closed {
		return nil, fmt.Errorf("ClickHouse DataSource runtime is closed")
	}
