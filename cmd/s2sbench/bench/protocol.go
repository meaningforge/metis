...
// physical_query parameters for DuckDB execution.
...
Return exactly the JSON object from the compile result's physical_query field, with its dialect, sql, and parameters fields unchanged.
...
Repair only the answer. If compile_sql already succeeded and the semantic request was correct, reuse that result without calling compile_sql again. Return exactly one complete physical_query JSON object and stop immediately after its closing brace. Do not use or infer any oracle/expected-result information.`
...
This is the current independent benchmark question in the same project. Use the same Metis MCP semantic interface. Return exactly the compile result's physical_query JSON object and no prose, markdown, or code fences.`
