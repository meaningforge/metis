...
func compileSemiAdditiveTieBreak(t *testing.T, query query.SemanticQuery, dialect, aggregation string) (*semanticplan.SemanticPlan, sql.SqlStatement) {
	t.Helper()
...
func compileSemiAdditiveWindowGrouping(t *testing.T, query query.SemanticQuery, dialect string) (*semanticplan.SemanticPlan, sql.SqlStatement) {
	t.Helper()
	doc, err := ossie.NewLoader().Load(fixtures.CommerceModelYAML)
	if err != nil {
		t.Fatal(err)
